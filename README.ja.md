...
**SnapTranslate** は、開発者や語学学習者のためのデスクトップスクリーンショット翻訳ツールです。画面上の任意の領域を選択するだけで、OCR 文字認識 + AI 翻訳を瞬時に実行。翻訳結果を元のコンテンツにオーバーレイ表示します——見たままの結果が得られます。

**コアコンセプト:** ワンクリックで領域選択 → その場に固定表示 → キャプチャしながら翻訳

> スクリーンショットは元の位置に固定表示されます — ポップアップ、画面遷移、ワークフローの中断は一切ありません。
...
| **テキスト翻訳** | `Ctrl+Alt+M` でシンプルなテキスト翻訳ウィンドウを開き、`Ctrl+Enter` で素早く翻訳 |
...
Ctrl+Alt+L を押す         領域を選択、スクリーンショットを元の位置に貼り付け
                               ↓
「翻訳」ボタンをクリック      OCR + AI 翻訳、結果を原文にオーバーレイ
                               ↓
