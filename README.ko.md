...
**SnapTranslate**는 개발자와 외국어 학습자를 위한 데스크톱 스크린샷 번역 도구입니다. 화면의 원하는 영역을 선택하면 즉시 OCR 문자 인식 + AI 번역을 수행하고, 번역 결과를 원본 콘텐츠 위에 오버레이하여 표시합니다 — 보이는 그대로의 결과를 얻을 수 있습니다.

**핵심 철학:** 원클릭 영역 선택 → 원위치 고정 → 캡처하며 바로 번역

> 스크린샷이 원래 위치에 고정됩니다 — 팝업, 화면 전환, 작업 흐름 중단이 없습니다.
...
| **텍스트 번역** | `Ctrl+Alt+M`으로 간결한 텍스트 번역 창 열기, `Ctrl+Enter`로 빠른 번역 |
| **로컬 OCR** | Tesseract 오프라인 엔진 내장, 인터넷 없이도 문자와 단락 위치 자동 인식 |
...
Ctrl+Alt+L 누르기          영역 선택, 스크린샷을 원래 위치에 고정
                              ↓
"번역" 버튼 클릭            OCR + AI 번역, 결과를 원문에 오버레이
                              ↓
...
- **macOS**: macOS 12+, WebKit (시스템 기본 포함)
- **Linux**: X11/Wayland 지원, WebKitGTK 필요
...
| OCR | Tesseract CLI (`chi_sim` + `eng` 언어 데이터 번들) |
