...
**SnapTranslate** 是一款面向開發者和外語學習者的桌面截圖貼圖翻譯工具。框選螢幕任意區域，瞬間完成 OCR 文字辨識 + AI 翻譯，譯文以貼圖形式覆蓋在原內容上，所見即所得。

**核心理念：** 一鍵框選 → 原位貼圖 → 隨截隨譯

> 截圖貼圖在原始位置 — 不彈窗、不跳轉、不打斷工作流。
...
| **文字翻譯** | `Ctrl+Alt+M` 開啟簡潔文字翻譯視窗，Ctrl+Enter 快捷翻譯 |
...
按 Ctrl+Alt+L         框選截圖並貼圖到原位
                         ↓
點選貼圖下方「翻譯」按鈕    OCR + AI 翻譯，譯文覆蓋在原文位置
                         ↓
點選「複製譯文」         將譯文複製到剪貼簿
                         ↓
