...
| S4   | 配置与历史     | 设置界面、密钥安全存储、翻译历史、国际化        | 功能完整的国际化版本          | 🔄 部分完成（设置已完成，历史和国际化待实现） |
...
- [ ] 截图翻译完成后自动生成历史记录（未实现）
- [ ] 历史面板可查看最近 50 条记录（未实现）
- [ ] 支持逐条删除和清空全部（未实现）
- [x] 剪贴板贴图不产生历史（无需额外逻辑）
- [ ] 界面文字跟随系统语言自动切换（中/英）（i18n 文件为空，待填充）
- [x] 快捷键可在设置中自定义

**实际完成情况：** 设置模块已完成（SettingsView.vue），包含 API 配置、连接测试、快捷键配置。历史记录模块（history/mod.rs）仅为注释占位，前端 HistoryView.vue 和 historyStore.ts 均为桩代码。国际化文件已创建但内容为空对象。
